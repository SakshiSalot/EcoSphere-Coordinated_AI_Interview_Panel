...
Passwords are generated and printed **once**. Write them down — only a hash is
...
| One interviewer — prove interruption works | `make gate` | `.venv\Scripts\python.exe -m scripts.day1_interrupt_test` |
...
alive — the agents then call an address that no longer exists and sit in the
...
*"how many marks was that question worth?"* — it depends only on the shape of
...
fix without a calibration set, so the *basis* — how many answers, how hard it
...
`scorer.PENALISED_KINDS` and cost exactly zero marks — the offline suite asserts
...
|---|---|
| Interviewer joins but never speaks | The tunnel's hostname stopped resolving — it does that while the process stays alive. Restart the tunnel and gateway (§5). |
